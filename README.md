...
這個不一致由兩條確定性規則抓出來，命中就在卡片上出現 ⚠（chips 的「手寫與實作不符」可以只看這些）：**`mentioned_untouched`** —— 進度檔提到、而且這個專案真的碰過的檔案，已經 30 天沒再被改（進度檔的目標／下一步還非空）；**`stale_memory`** —— 進度檔比專案最後一次改檔舊 14 天以上，期間卻改了 ≥10 次檔。基準時間取「這個專案最後一筆 `file_touch`」而不是系統時鐘：用系統時鐘的話，停擺一年的專案會整排變紅，那講的是「沒人動」不是「寫的跟做的不一樣」。
